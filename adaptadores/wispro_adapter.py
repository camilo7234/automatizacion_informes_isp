...
class WisproAdapter:
    ...
    def _mapear_periodo(self):
        ...
        # Validación mínima inmediata
        if not self.modelo["periodo"]["anio"] or not self.modelo["periodo"]["mes"]:
            raise ValueError("No se pudo determinar el periodo (anio/mes)")
    # --------------------------------------------------
    # USUARIOS
    # --------------------------------------------------
    def _mapear_usuarios(self):
        ...
        self.modelo["usuarios"]["pendientes_instalacion"] = sum(
            1 for u in usuarios if u.get("estado") == "PENDIENTE_INSTALACION"
        )
    ...
    def _mapear_inventario(self):
        ...
        # Normalizar campos al esquema contractual
        self.modelo["inventario_cpe"]["detalle"] = [
            {
                "serial": item.get("serial") or item.get("numero_serie"),
                "marca": item.get("marca"),
                "modelo": item.get("modelo"),
                "estado": item.get("estado"),
                "fecha_asignacion": item.get("fecha_asignacion") or item.get("fecha_instalacion"),
            }
            for item in inventario if isinstance(item, dict)
        ]
    # --------------------------------------------------
    # SERVICIO
    # --------------------------------------------------
    def _mapear_servicio(self):
        ...
        incidentes = servicio.get("incidentes", [])
        detalle = []
        for inc in incidentes:
            if not isinstance(inc, dict):
                continue
            detalle.append({
                "usuario_id": inc.get("id_usuario") or inc.get("usuario_id"),
                "fecha_inicio": inc.get("fecha_inicio"),
                "fecha_fin": inc.get("fecha_fin"),
                "duracion_horas": inc.get("duracion_horas"),
                "causa": inc.get("causa"),
            })
        self.modelo["servicio"]["usuarios_fuera_servicio"] = detalle
    # --------------------------------------------------
    # PQRS
    # --------------------------------------------------
    def _mapear_pqrs(self):
        ...
        for item in pqrs:
            ...
            detalle.append({
                "usuario_id": item.get("id_usuario") or item.get("usuario_id"),
                "tipo": item.get("tipo"),
                "fecha_radicado": item.get("fecha_radicado"),
                "estado": item.get("estado"),
            })

        self.modelo["pqrs"]["detalle"] = detalle
    # --------------------------------------------------
    # INDICADORES
    # --------------------------------------------------
