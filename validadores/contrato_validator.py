...
class ContratoValidator:
    ...
    def validar_esquema_modelo(self):
        ...
        def extraer_rutas_validacion(dic, prefijo=""):
            ...
            for k, v in dic.items():
                ...
                if k in ["modo_validacion", "reglas_generales", "obligatorio", "_tipo", "campos"]:
                    continue

                ruta = f"{prefijo}.{k}" if prefijo else k
                rutas.add(ruta)

                # Si es un bloque de lista, no expandir "campos" como rutas del modelo
                if isinstance(v, dict) and v.get("_tipo") == "lista":
                    continue
    ...
    def _validar_texto_generico(self):
        textos_prohibidos = (
            self.validaciones
            .get("reglas_generales", {})
            .get("texto_generico_prohibido", [])
        )
    ...
    def _valor_invalido(self, valor):
        ...
        # Las listas vacías son válidas (ej: 0 instalaciones en el mes)
        return False